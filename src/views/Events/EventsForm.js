...
export default function EventsForm(props) {
  ...
  React.useEffect(() => {
    async function fetchAgenda() {
      ...
      let snapshot = await firebase
        .database()
        .ref(`/regionais/ribeirao-preto/dados/lista/${month}/${id}`)
        .once("value");
    ...
    fetchAgenda();
  }, [props.match.params.id, props.match.params.month]);
  ...
  const validate = () => {
    ...
    if (service === "") validations.push("Serviço é obrigatório.");
  ...
  const saveAgenda = async () => {
    ...
    try {
      ...
      if (id === null)
        await firebase
          .database()
          .ref(`/regionais/ribeirao-preto/dados/lista/${month}`)
          .push()
          .set(agenda);
      else
        await firebase
        ...
        <GridItem xs={12} sm={12} md={8}>
          <Card>
            <CardHeader color="primary">
              <h4 className={classes.cardTitleWhite}>Informações do Serviço</h4>
            </CardHeader>
            <CardBody>
              <GridContainer>
              <GridItem xs={12} sm={12} md={6}>
                  <FormControl className={classes.formControl}>
                    <InputLabel id="select">Serviço</InputLabel>
                    <Select
                      labelId="select"
                      id="select"
                      value={service}
